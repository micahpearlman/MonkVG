...
void OpenGLPaint::buildLinearGradientImage(VGfloat pathWidth,
                                           VGfloat pathHeight) {
    ...
    //
    VGfloat p0[2] = {(_paintLinearGradient[0] / pathWidth) * width,
                     (_paintLinearGradient[1] / pathHeight) * height};
    VGfloat p1[2] = {(_paintLinearGradient[2] / pathWidth) * width,
                     (_paintLinearGradient[3] / pathHeight) * height};
    ...
    vgImageSubData(_gradientImage, image, -1, VG_sRGBA_8888, 0, 0, width,
                   height);

    free(image);
}

void OpenGLPaint::buildRadialGradientImage(VGfloat pathWidth,
                                           VGfloat pathHeight) {
    // generated image sizes
    const int     width = 64, height = 64;
    unsigned int *image =
...
void OpenGLPaint::buildLinear2x3GradientImage(VGfloat pathWidth,
                                              VGfloat pathHeight) {
    // generated image sizes
    const int     width = 64, height = 64;
    unsigned int *image =
    ...
    // gradients float delta = (pathWidth-gradientWidth) / pathWidth; VGfloat
    // p0[2] = { (delta/2) * width, 0}; VGfloat p1[2] = { p0[0] +
    // ((gradientWidth / pathWidth) * width), 0};
    ...
    for (int x = 0; x < width; x++) {
        for (int y = 0; y < height; y++) {
            ...
            if (spreadMode == VG_COLOR_RAMP_SPREAD_PAD) {
                ...
                } else if (g < _colorRampStops[0]
                                   .a[0]) { // if the first color is after 0
                ...
                } else if (g > _colorRampStops[stopCnt - 1]
                                   .a[0]) { // if the last color is before 1.0
...
void OpenGLPaint::buildRadial2x3GradientImage(VGfloat pathWidth,
                                              VGfloat pathHeight) {
    // generated image sizes
    const int     width = 64, height = 64;
    unsigned int *image =
