...
void IFont::addGlyphImage(VGuint index_, IImage *image_,
                          VGfloat glyphOrigin_[2], VGfloat escapement_[2]) {
    _glyphs[index_] = std::make_unique<IFont::GlyphImage>(
        index_, image_, glyphOrigin_, escapement_);
}

void IFont::addGlyphPath(VGuint index_, IPath *path_, VGfloat glyphOrigin_[2],
                         VGfloat escapement_[2]) {
    _glyphs[index_] = std::make_unique<IFont::GlyphPath>(
...
void IFont::GlyphImage::draw(VGbitfield paintModes, VGfloat adj_x,
    ...
    _image->drawAtPoint(origin[0], origin[1], paintModes);
}
...
VG_API_CALL VGFont VG_API_ENTRY vgCreateFont(VGint glyphCapacityHint)
    VG_API_EXIT {
...
VG_API_CALL void VG_API_ENTRY vgSetGlyphToPath(
    VGFont font, VGuint glyphIndex, VGPath path, VGboolean isHinted,
    VGfloat glyphOrigin[2], VGfloat escapement[2]) VG_API_EXIT {
...
VG_API_CALL void VG_API_ENTRY
vgDrawGlyphs(VGFont font, VGint glyphCount, VGuint *glyphIndices,
             VGfloat *adjustments_x, VGfloat *adjustments_y,
             VGbitfield paintModes, VGboolean allowAutoHinting) VG_API_EXIT {
